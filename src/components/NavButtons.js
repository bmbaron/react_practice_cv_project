...
class NavButtons extends Component {
	...
	displaySample = () => {
		this.props.loadSample(!this.state.sample)
		this.setState({
	...
	render() {
		...
		const displaySampleText = this.state.show ? '' : 'Load sample'
		return(
			<div className='next-section-container'>
				...
				{!this.state.show && 
					<h2 className='sample-cv' onClick={this.displaySample}>{displaySampleText}</h2>
				}
